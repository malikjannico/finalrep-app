...
class _CompetitionHandlingPageState extends State<CompetitionHandlingPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<CompetitionProvider>().initCompetitionHandling(widget.competitionId ?? '');
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final otherPlatesStr = StreetliftingRulesEngine.calculateOtherPlatesString(attemptWeight);
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ElevatedButton(
                  ...
                  onPressed: isDisqualified ? null : () => provider.toggleJudgeVote(0),
                  child: Text('J1: ${judgeVotes[0] ? "Good" : "No"}'),
                ),
                ElevatedButton(
                  key: const Key('judge_2_toggle'),
                  onPressed: isDisqualified ? null : () => provider.toggleJudgeVote(1),
                  child: Text('J2: ${judgeVotes[1] ? "Good" : "No"}'),
                ),
                ElevatedButton(
                  key: const Key('judge_3_toggle'),
                  onPressed: isDisqualified ? null : () => provider.toggleJudgeVote(2),
            ...
            DropdownButton<String>(
              ...
              items: ['Chicken Wing', 'Invalid Depth', 'Bent Knees', 'Kipping']
                  ...
                  .toList(),
              onChanged: isDisqualified ? null : (val) => provider.setFailureReason(val),
            ),
            ElevatedButton(
