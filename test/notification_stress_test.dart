...
class MockCompetitionRepository implements CompetitionRepository {
  ...
  Future<bool> registerAthlete(String competitionId, String userId) async {
    ...
    if (!list.any((a) => a.id == userId)) {
      list.add(Profile(id: userId, username: 'user-$userId', fullName: 'Athlete $userId', email: '$userId@test.com'));
...
class MockAdminRepository implements AdminRepository {
  ...
  @override
  Future<PermissionApplication?> applyForPermissions(String userId, String type, String reason) async {
...
void main() {
  group('Notification System Trigger Stress Tests', () {
    ...
    test('1. Registration Trigger Fires Successfully', () async {
      ...
      // Get notifications
      final notifications = await notificationRepo.getNotifications('athlete-123');
    ...
    test('2. Permission Approval/Rejection Triggers Fire Successfully', () async {
      // 2.1 Approval Trigger
      final app = await mockAdminRepo.applyForPermissions('athlete-123', 'create_competition', 'To organize regional meets.');
      expect(app, isNotNull);

      // Approve application
      await authProvider.approvePermissionApplication(app!.id);

      final notifsApproval = await notificationRepo.getNotifications('athlete-123');
      expect(notifsApproval.any((n) => n.category == 'permissions' && n.title == 'Permissions Approved'), true);

      // 2.2 Rejection Trigger
      final app2 = await mockAdminRepo.applyForPermissions('athlete-123', 'create_association', 'To run federation.');
      expect(app2, isNotNull);

      // Reject application
      await authProvider.rejectPermissionApplication(app2!.id);

      final notifsRejection = await notificationRepo.getNotifications('athlete-123');
      expect(notifsRejection.any((n) => n.category == 'permissions' && n.title == 'Permissions Application Update'), true);
    });

    test('3. Payment Triggers (Formulation & Registration Action Required) Fire Successfully', () async {
      ...
      final ownerNotifs = await notificationRepo.getNotifications('assoc-owner-999');
      ...
      final athleteNotifs = await notificationRepo.getNotifications('athlete-fee-test');
    ...
    test('4. Schedule Release Trigger Fires Successfully', () async {
      ...
      // Register athlete
      await compProvider.registerAthlete(competitionId: comp.id, userId: 'athlete-123');

      // Publish schedule
      await compProvider.publishSchedule(comp.id, isPublic: true);

      final notifs = await notificationRepo.getNotifications('athlete-123');
      expect(notifs.any((n) => n.category == 'schedule' && n.title == 'Meet Schedule Published'), true);
    });
    ...
    test('5. Flight Assignment Trigger Fires Successfully', () async {
      ...
      final athlete1Notifs = await notificationRepo.getNotifications('athlete-1');
      expect(athlete1Notifs.any((n) => n.category == 'flights' && n.title == 'Flight Assignment Updated' && n.message.contains('Flight A')), true);

      // Verify athlete 13 got assigned to Flight B
      final athlete13Notifs = await notificationRepo.getNotifications('athlete-13');
      expect(athlete13Notifs.any((n) => n.category == 'flights' && n.title == 'Flight Assignment Updated' && n.message.contains('Flight B')), true);
    });
    ...
    test('6. Volunteer Application Trigger Fires Successfully', () async {
      ...
      final volunteerNotifs = await notificationRepo.getNotifications('volunteer-1');
  ...
  group('Profile Notification Preferences Deserialization & Sync', () {
    test('1. Partial preferences JSON correctly parses and merges with default switches', () {
      final json = {
        'id': 'user-partial-123',
        'username': 'partialguy',
      ...
      expect(profile.notificationPreferences['schedule'], true); // default fallback
      expect(profile.notificationPreferences['flights'], true);    // default fallback
    });

    test('2. Missing preferences JSON correctly initializes all toggles to true', () {
      final json = {
        'id': 'user-missing-123',
        'username': 'missingguy',
  ...
  group('NotificationsPage Widget Stress Tests', () {
    ...
    testWidgets('1. Settings toggles correctly filter notifications on display', (WidgetTester tester) async {
      ...
      expect(find.text('Hamburg Registration'), findsOneWidget);
      expect(find.text('Munich Payment'), findsNothing); // Payments notification should be filtered out!
    });

    testWidgets('2. Category chips filter notifications correctly', (WidgetTester tester) async {
