...
void main() {
  group('Streetlifting Rules & Competition Handling Adversarial Tests', () {
    ...
    test('Adversarial Test 1: VAR overrule of failed 3rd attempt causes state corruption when prior attempt succeeded', () {
      ...
      expect(provider.submittedAttempts, isEmpty, reason: 'Overruled weight from prior discipline must not pollute next discipline');
    });

    test('Adversarial Test 2: Decreasing weight attempts after failed attempts are permitted', () {
      final provider = harness.competitionProvider;
      provider.initCompetitionHandling('comp-1');
      ...
      final error = provider.selectAttemptWeight('athlete-1', 'Muscle Up', 2, 15.0);
      
      // If the bug exists, error will be null (it is accepted)!
      debugPrint('ERROR FOR LIGHTER ATTEMPT: $error');
      expect(error, isNotNull, reason: 'Lighter attempt after failed attempt must be blocked');
    });
  });
}
