...
void main() {
  ...
  test('Inspect profiles table schema', () async {
    ...
    try {
      final response = await client.from('profiles').select().eq('username', 'nonexistent_test_user_123').maybeSingle();
      print('DB RESPONSE FOR SELECT: $response');
    } catch (e) {
      print('DB INSPECT ERROR: $e');
      if (e is PostgrestException) {
        print('PostgrestException details: ${e.message}, ${e.code}, ${e.details}, ${e.hint}');
      }
    }
  });
}
