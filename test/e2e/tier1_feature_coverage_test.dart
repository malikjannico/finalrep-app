  ...
  group('E2E Tier 1: Feature Coverage Tests', () {
    ...
    group('Feature Area 1: Authentication & Password', () {
      testWidgets('Test 1.1: Username entry dynamically lowercases and verifies successfully', (WidgetTester tester) async {
        ...
        debugPrint('DEBUG TEST 1.1 AFTER PUMP: isAuthenticated=${harness.authProvider.isAuthenticated} status=${harness.authProvider.status}');
      ...
      testWidgets('Test 1.2: Successful login with correct Username + Password credentials', (WidgetTester tester) async {
        await harness.initialize();
        await tester.pumpWidget(harness.buildApp(const LoginPage(isInline: true)));
        await tester.pumpAndSettle();
      ...
      testWidgets('Test 1.3: Successful login with correct Email + Password credentials', (WidgetTester tester) async {
        await harness.initialize();
        await tester.pumpWidget(harness.buildApp(const LoginPage(isInline: true)));
        await tester.pumpAndSettle();
        ...
        expect(harness.authProvider.isAuthenticated, true);
        expect(harness.authProvider.currentUserProfile?.email, 'john@example.com');
      });

      testWidgets('Test 1.4: Initiating forgot password recovery sends the reset link using the email form', (WidgetTester tester) async {
        await harness.initialize();
        await tester.pumpWidget(harness.buildApp(const LoginPage(isInline: true)));
        await tester.pumpAndSettle();
      ...
      testWidgets('Test 1.5: Logout successfully resets current user profile and returns app back to guest status', (WidgetTester tester) async {
        await harness.initialize();
    ...
    group('Feature Area 2: Profile Customization', () {
      testWidgets('Test 2.1: Render profile details directly on page scaffold without enclosing Card borders', (WidgetTester tester) async {
        await harness.initialize();
        // Authenticate user-1
        final user = harness.db.profiles['user-1']!;
        final session = Session(
      ...
      testWidgets('Test 2.2: Settings gear icon is positioned immediately inline following the user\'s Full Name', (WidgetTester tester) async {
        await harness.initialize();
        // Authenticate user-1
        final user = harness.db.profiles['user-1']!;
        final session = Session(
      ...
      testWidgets('Test 2.3: Toggle edit mode and update profile parameters', (WidgetTester tester) async {
        ...
        // Edit fields
        await tester.enterText(find.widgetWithText(TextFormField, 'Description / Bio'), 'New lifting bio!');
        await tester.pumpAndSettle();
      ...
      testWidgets('Test 2.4: Upload custom profile avatar image, previewing and verifying updated public storage URL', (WidgetTester tester) async {
        tester.view.physicalSize = const Size(800, 1200);
        tester.view.devicePixelRatio = 1.0;
        addTearDown(() {
          tester.view.resetPhysicalSize();
        ...
        harness.mockFilePicker.setMockFile('my_avatar.png', 1024, transparentPngBytes);
        ...
        debugPrint('DEBUG TEST 2.4: changePhotoFinder.evaluate() elements length = ${changePhotoFinder.evaluate().length}');
        if (changePhotoFinder.evaluate().isNotEmpty) {
          final renderBox = tester.renderObject(changePhotoFinder) as RenderBox;
          final position = renderBox.localToGlobal(Offset.zero);
          final size = renderBox.size;
          debugPrint('DEBUG TEST 2.4: CHANGE PHOTO position=$position size=$size');
        }
      ...
      testWidgets('Test 2.5: Navigate to another user\'s profile view', (WidgetTester tester) async {
        await harness.initialize();
        // Log in user-1
        final user = harness.db.profiles['user-1']!;
        final session = Session(
    ...
    group('Feature Area 3: Competitions Feed & Details', () {
      testWidgets('Test 3.1: Switch feed view dynamically between layouts (Grid / List / Map)', (WidgetTester tester) async {
        await harness.initialize();
      ...
      testWidgets('Test 3.3: Active filter chips are rendered above feed', (WidgetTester tester) async {
        await harness.initialize();
        tester.view.physicalSize = const Size(1200, 800);
        tester.view.devicePixelRatio = 1.0;
        addTearDown(() {
      ...
      testWidgets('Test 3.4: Sorting order toggles correctly', (WidgetTester tester) async {
        await harness.initialize();
        tester.view.physicalSize = const Size(1200, 800);
        tester.view.devicePixelRatio = 1.0;
        addTearDown(() {
      ...
      testWidgets('Test 3.5: Competition detail view navigation renders content', (WidgetTester tester) async {
        await harness.initialize();
        tester.view.physicalSize = const Size(1200, 800);
        tester.view.devicePixelRatio = 1.0;
        addTearDown(() {
